...
function openEditModal(id){
  ...
  if(typeof populateColecaoSelects==='function') populateColecaoSelects(p.colecao||'');
  var colEl = document.getElementById('admColecao');
  if(colEl && p.colecao) colEl.value = p.colecao;
...
function saveProduct(){
  ...
  const colecao = document.getElementById('admColecao')?.value||'';
  ...
  if(editingProductId){
    ...
    if(idx>=0){
      liveProducts[idx]={...liveProducts[idx],name,cat,colecao,cl:(typeof getCatLabel==='function'?getCatLabel(cat):CAT_LABELS[cat]||cat),price,old,badge,sizes,img,desc};
    }
  } else {
    const newId = 'custom_'+Date.now();
    liveProducts.push({id:newId,name,cat,colecao,cl:(typeof getCatLabel==='function'?getCatLabel(cat):CAT_LABELS[cat]||cat),price,old,badge,sizes,img,desc});
  }
  if(typeof saveToStorage==='function') saveToStorage();
  renderAdminGrid();
  updateStats();
